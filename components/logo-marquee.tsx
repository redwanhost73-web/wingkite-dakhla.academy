...
// Duplicate enough times so the strip never gaps on wide screens.
const STRIP = Array.from({ length: 8 }, () => LOGOS).flat()
      ...
      <div className="relative">
        ...
        <div className="flex w-max animate-logo-marquee items-center gap-16 px-10 sm:gap-24">
          {STRIP.map((logo, i) => (
            <div
              key={`${logo.src}-${i}`}
              className="relative h-9 shrink-0 opacity-40 grayscale transition-[opacity,filter] duration-[600ms] ease-[cubic-bezier(0.16,1,0.3,1)] hover:opacity-100 hover:grayscale-0 sm:h-11"
